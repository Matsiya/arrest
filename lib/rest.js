...
RestAPI.prototype._save = function(req, res) {
    if (!req.params.id) {
        ...
        this.save(req.params.id, req.body, exports.responseCallback(res));
    }
}
...
RestCouchBaseAPI.prototype.query = function(ddoc, view, query, cb) {
    ...
    self.db.view(ddoc, view).query(query, function(err, results) {
        ...
        for( var i = 0; i < l; i++ ) {
            ...
            for (var key in result.value ) { formatedResult[key] = result.value[key]; }


            formattedResults.push( formatedResult );
        }


        self._return(err, formattedResults, true, cb );
    });
}
RestCouchBaseAPI.prototype.get = function(key, options, cb) {
    ...
    self.db.get(key, options, function(err, data) {
        if( data.value && data.value._sync )
            delete data.value["_sync"];

        self._return(err, data.value, false, cb);
    });
}
RestCouchBaseAPI.prototype.save = function(id, data, cb) {
    var self = this;

    var options = {
        uri: this.options.syncGatewayHost.host + this.options.bucket + '/' + id,
        method: 'PUT',
        json: data
    };
    ...
    }).auth(this.options.syncGatewayHost.username, this.options.syncGatewayHost.password, false);
