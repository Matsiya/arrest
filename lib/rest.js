...
function RestCouchBaseAPI(hosts, bucket, password, designDoc ) {
    RestAPI.call(this);
    ...
    if( hosts && hosts.length > 0 ) {
        connectOptions.host = hosts;
    }

    if( bucket && bucket.length > 0 ) {
        connectOptions.bucket = bucket;
    }
    ...
    this.designDoc = designDoc || this.bucket;
...
RestCouchBaseAPI.prototype.query = function(criteria, options, cb) {
